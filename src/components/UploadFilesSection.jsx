...
export const UploadFilesSection = () => {
  ...
  const currentStep = !companyData?.length ? 'company' : !bankData?.length ? 'bank' : 'complete';
  ...
  return (
    <div className="upload-files-container">
      ...
      <div className="upload-steps">
        <div className={`step ${companyData?.length ? 'completed' : currentStep === 'company' ? 'active' : ''}`}>
        ...
        <div className={`step ${bankData?.length ? 'completed' : currentStep === 'bank' ? 'active' : ''}`}>
          <div className="step-indicator">
            {bankData?.length ? '✓' : '2'}
          </div>
          ...
          <div className="upload-content">
            {loading ? (
              <>
                ...
                <h3>Processing {type === 'company' ? 'Company' : 'Bank'} file...</h3>
                <p>Please wait</p>
              </>
            ) : (
              <>
                <div className="upload-icon">{type === 'company' ? '🏢' : '🏦'}</div>
                <h3>Upload {type === 'company' ? 'Company' : 'Bank'} Excel File</h3>
                <p className="upload-hint">Click here or drag and drop your .xlsx or .xls file</p>
                <input
                  id={`${type}-file-input`}
                  type="file"
                  accept=".xlsx,.xls"
          ...
          <div className="files-list">
            {companyFileName && (
              <div className="file-item company-file">
                ...
                <button onClick={() => clearData('company')} className="remove-file-btn">
                  ✕
                </button>
              </div>
            )}
            {bankFileName && (
              <div className="file-item bank-file">
                ...
                <button onClick={() => clearData('bank')} className="remove-file-btn">
                  ✕
                </button>
              </div>
            )}
